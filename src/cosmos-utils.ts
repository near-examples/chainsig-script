...
export class CosmosUtils {
  ...
  async buildSimpleMsgSendStdSignDoc(
    ...
    return this.buildSimpleStdSignDoc(
      { accountNumber: account.account_number, sequence: account.sequence },
      [msgSendEncoded]
    );
  }
  ...
  private async buildSimpleStdSignDoc(
    ...
    const signDoc = makeSignDoc(
      aminoMessages,
      {
        amount: [{ amount: toAmount(0.001).toString(), denom: this.denom }],
        gas: "200000",
      },
      this.chainId,
      undefined,
  ...
  async broadcastCosmosAmino(signedDoc: StdSignDoc, signature: StdSignature) {
    ...
    const result = await client.broadcastTxSync(TxRaw.encode(txRaw).finish());
    return result;
  }
